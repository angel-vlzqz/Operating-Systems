...
private:
    ...
    {
        if (!directory)
            return; // Safety check to prevent dereferencing nullptr
...
public:
    ...
    File *findFile(const string &path) const
    {
        ...
        while (pos != string::npos && pos < path.size())
        {
            ...
            if (it == current->children.end())
                return NULL;                                              // No such path part exists
    ...
    bool duplicateFile(const string &sourcePath, const string &targetPath)
    {
        ...
        if (sourceFileNameIndex == string::npos || targetFileNameIndex == string::npos)
            return false;
